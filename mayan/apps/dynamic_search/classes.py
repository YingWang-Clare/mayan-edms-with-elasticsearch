...
class SearchModel(object):
    ...
    # Mapping search field of DocumentPageResult to Document
    document_to_page = {
        "document_type__label": "document_type__label",
        "versions__mimetype": "versions__mimetype",
        "label": "label",
        "description": "description",
        "versions__checksum": "versions__checksum",
        "document_version__document__cabinets__label": "cabinets__label",
        "document_version__document__comments__comment": "comments__comment",
        "content__content": "versions__pages__content__content",
        "document_version__document__metadata__metadata_type__name": "metadata__metadata_type__name",
        "document_version__document__metadata__value": "metadata__value",
        "ocr_content__content": "versions__pages__ocr_content__content",
        "document_version__document__tags__label": "tags__label"
    }

    def elastic_search(self, query_string, user, global_and_search=False):
        es = self.initialize_es()
        ...
        logger.info('search dict: %s', search_dict)
        result_set = set()
        for model, data in search_dict.items():
            ...
            logger.info('search_list: %s', search_list)

            for id, ele in enumerate(search_list): # id=0, ele={u'terms': [u'test'], u'field_name': [u'label']}
                field_result_set = set()
                for term in ele['terms']: # ['term1', 'term2', ...]
                    term_query_result_set = set()
                    if model.__name__ == "Document":
                        logger.info('es query: %s : %s', ele['field_name'][0], term)
                        results = es.search(index=settings.ELASTIC_SEARCH_INDEX, body={
                            "query": {
                                "match": {ele['field_name'][0]: term}
                            }
                        })
                        for result in results['hits']['hits']:
                            term_query_result_set.add(int(result['_source']['pk']))
                    elif model.__name__ == "DocumentPageResult":
                        logger.info('es query: %s : %s', self.document_to_page[ele['field_name'][0]], term)
                        results = es.search(index=settings.ELASTIC_SEARCH_INDEX, body={
                            "query": {
                                "match": {self.document_to_page[ele['field_name'][0]]: term}
                            }
                        })
                        for result in results['hits']['hits']:
                            term_query_result_set.add(int(result['_source']['versions__pages__pk']))
        ...
        logger.debug('result set: %s', result_set)
